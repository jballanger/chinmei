...
class Chinmei extends EventEmitter{
	...
	getMalUser(user, type = 1, status = 'all') {
		let self = this;
		let t = (type == 1 ? 'anime' : 'manga');
		return new Promise((resolve, reject) => {
			request({url: `https://myanimelist.net/malappinfo.php?u=${user}&type=${t}&status=${status}`}, function(err, res, body) {
				if (err) reject(err);
				else if (res.statusCode !== 200) reject(res.statusCode);
				self.utils.formatMalUser(body, t).then((MalUser) => {
					resolve(MalUser);
				}).catch((e) => {
					reject(e);
				});
