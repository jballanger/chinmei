...
exports.formatMalUser = (body, type) => {
	return new Promise((resolve, reject) => {
		xml2js.parseString(body, async function(err, res) {
			if (err) reject(err);
			let user = JSON.parse(JSON.stringify(res)).myanimelist;
			if (user.length < 1) reject(new Error('User not found'));
			else {
				user.myinfo = user.myinfo[0];
				await Object.keys(user.myinfo).map(k => user.myinfo[k] = user.myinfo[k][0]);
				await Object.keys(user[type]).map(k => Object.keys(user[type][k]).map(t => user[type][k][t] = user[type][k][t][0]));
				resolve(user);
			}
		});
	});
